...
const preprocess = sveltePreprocess({postcss : {plugins : plugins}});
  ...
  client: {
    ...
    plugins:
        [
          ...
          !dev && terser({module : true}), visualizer()
        ],
    onwarn
  },
  ...
  server: {
    ...
    plugins:
        [
          ...
          svelte({preprocess, generate : 'ssr', dev}), postcss({
            extract : './static/global.css',
            plugins : [
              require('postcss-import'),
              require('tailwindcss'), // See tailwind.config.js
              require('autoprefixer'), require('postcss-fail-on-warn'),
              // Do not purge the CSS in dev mode to be able to play with
              // classes in the browser dev-tools.
              !dev && purgecss, !dev && require('cssnano')({preset : 'default'})
            ].filter(Boolean)
          }),
          resolve({dedupe}), commonjs(), json(), typescript()
        ],
