<script lang="ts">
	...
	async function login() {
		try {
			...
			firebase
				...
				.then(() => (currentlyAuthenticating = false))
				.catch(err => {
	...
	async function updateList() {
		...
		const res = await fetch(
			'https://moosik-backend.herokuapp.com/playlist/post',
			{
				method: 'POST',
				body: JSON.stringify({
					uid: user.user.uid,
	...
	async function downloadList() {
		...
		const res = await fetch(
			'https://moosik-backend.herokuapp.com/playlist/get',
			{
				method: 'POST',
				body: JSON.stringify({
					uid: user.user.uid
...
{#if user}
	<img
		...
		class="rounded-full mx-auto mt-6 w-20 h-20" />
	<Title1 content={user.user.displayName} />
	<div class="w-full clearfix">
		<form
			class="bg-white shadow-md clearfix rounded px-8 pt-6 pb-8 mb-4 main-div">
			<div class="mb-4">
				<label
...
{:else if currentlyAuthenticating}
	<Title2 content={'authenticating...'} />
{:else}
	{#if authError}
		<Title2 content={authError} />
	{/if}
	<button class="title2 mx-auto self-center" on:click={login}> login </button>
{/if}
